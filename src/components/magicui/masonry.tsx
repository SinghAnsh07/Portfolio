...
const Masonry: React.FC<MasonryProps> = ({
      ...
      await Promise.all(
        items.map(item =>
          new Promise<void>((resolve) => {
            const img = new Image();
            img.onload = () => {
              dimensions[item.id] = { width: img.width, height: img.height };
              resolve();
            };
            img.onerror = () => {
              dimensions[item.id] = { width: 800, height: 600 }; // fallback
              resolve();
            };
            img.src = item.img;
          })
  ...
  const grid = useMemo<GridItem[]>(() => {
    if (!width || Object.keys(imageDimensions).length === 0) return [];
    ...
    return items.map(child => {
      ...
      const aspectRatio = imgDim ? imgDim.height / imgDim.width : 1;
